...
(() => {
  ...
  function getAutoObjectPosition(metrics) {
    ...
    if (
      metrics.ratio < HERO_CROP_PORTRAIT_RATIO ||
      metrics.verticalCrop > HERO_CROP_SAFE_VERTICAL_CROP
    ) {
      return '50% 25%';
    }
    if (
  ...
  function applyHeroVideoCrop(container, video, item) {
    ...
    const focalPosition =
      normaliseObjectPosition(
        item?.objectPosition || item?.heroObjectPosition || item?.crop?.objectPosition
      ) ||
    ...
    container.style.setProperty(
      '--hv3-video-object-fit',
      cropMode === 'safe' ? 'contain' : 'cover'
    );
  }
  ...
  function normaliseSelectedVideoItem(item, fallbackId = 'selected-video') {
    ...
    return {
      ...
      objectPosition:
        item.objectPosition || item.heroObjectPosition || item.crop?.objectPosition || null,
  ...
  function setHeroVideoSource({ container, video, source, item, shouldPlay }) {
    ...
    const applySource = () => {
      ...
      source.type =
        item.file.file_type || item.file.fileType || getVideoTypeFromUrl(item.file.link);
