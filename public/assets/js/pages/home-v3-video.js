...
(() => {
    ...
    function getAutoObjectPosition(metrics) {
        ...
        if (metrics.ratio < HERO_CROP_PROTECT_TOP_RATIO ||
    ...
    function applyHeroVideoCrop(container, video, item) {
        ...
        const focalPosition = normaliseObjectPosition(item?.objectPosition || item?.heroObjectPosition || item?.crop?.objectPosition) ||
    ...
    function chooseHeroVideoFile(video, settings = {}) {
        ...
        const qualityFiles = preference === 'hd' || preference === 'sd'
    ...
    function getRotationDelay(settings = {}) {
        ...
        const multiplier = isMobileViewport() && settings.mobileOptimizations?.slowerTransitions === true
    ...
    function applyTransitionSettings(container, settings = {}) {
        ...
        const mobileDurationMultiplier = isMobileViewport() && settings.mobileOptimizations?.slowerTransitions === true
            ? MOBILE_TRANSITION_MULTIPLIER
            : 1;
    ...
    function getSourceSwapFadeDelay(container) {
        ...
        return Math.max(DEFAULT_SOURCE_SWAP_FADE_MS, Math.min(duration * SOURCE_SWAP_FADE_RATIO, MAX_SOURCE_SWAP_FADE_MS));
    }
    ...
    async function fetchPexelsPlaylist(settings) {
        ...
        const response = await fetch(`/api/v1/public/homepage-video?version=v3&query=${encodeURIComponent(query)}`, {
            credentials: 'same-origin',
            headers: {
    ...
    async function initialisePexelsHeroVideo() {
        ...
        const canFetchPexels = settings.source === 'pexels' ||
