(function () {
  ...
  async function loadRequests() {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error('Failed to load cashout requests');
      }
  ...
  function openModal(id, items) {
    ...
    // Pre-populate fields
    if (responseMsg) {
      responseMsg.value = r.adminResponseMessage || '';
    }
    if (internalNotes) {
      internalNotes.value = r.adminInternalNotes || '';
    }
    if (statusSelect) {
      statusSelect.value = '';
    }
  ...
  async function saveChanges() {
    ...
    try {
      ...
      if (adminResponseMessage !== undefined) {
        body.adminResponseMessage = adminResponseMessage;
      }
      if (adminInternalNotes !== undefined) {
        body.adminInternalNotes = adminInternalNotes;
      }
