(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      .customer-wedding-card.ww-card-launcher-only .customer-wedding-card-header,
      ...
      .customer-wedding-card.ww-card-launcher-only .cd-card-toggle {
        display: none !important;
      }

      .customer-wedding-card.ww-card-launcher-only .cd-card-body {
      ...
      .ww-unseated-panel .ww-unseated,
      .ww-table-board .seat-grid {
        margin-top: 0.65rem;
      }
      ...
      .ww-table-card.is-drop-target {
        border-color: rgba(80, 192, 176, 0.58) !important;
        box-shadow: 0 18px 42px rgba(44, 148, 177, 0.16) !important;
      ...
      .ww-capacity-bar {
        ...
        overflow: hidden;
        border-radius: 999px;
        background: rgba(80, 192, 176, 0.12);
        margin: 0.55rem 0;
      }

      .ww-capacity-bar span {
        display: block;
        height: 100%;
      ...
      .ww-qr-placeholder span {
        border-radius: 8px;
        background: linear-gradient(135deg, var(--ww-navy, #24436f), var(--ww-teal, #2c94b1));
      }
  ...
  function enhanceStats(root) {
    root.querySelectorAll('.ww-tiles > div').forEach(card => {
  ...
  function enhanceShare(root) {
    ...
    if (!input || !actions || root.querySelector('.ww-qr-card')) {
      return;
    }
    ...
    extra.innerHTML = `
      <div class="ww-actions">
        <a class="cta secondary small" target="_blank" rel="noopener" href="https://wa.me/?text=${encodedUrl}">Share to WhatsApp</a>
        <a class="cta secondary small" href="mailto:?subject=Wedding%20details&body=${encodedUrl}">Share by email</a>
      </div>
      <div class="ww-qr-card">
        <div class="ww-qr-placeholder" aria-hidden="true"><span></span><span></span><span></span><span></span></div>
        <p>QR code placeholder</p>
        <small>Designed for a lightweight QR utility in the next pass.</small>
      </div>
    `;
    actions.after(extra);
  ...
  function enhanceSeating(root) {
    ...
    if (!panel || !grid || !unseated || panel.classList.contains('ww-seating-enhanced')) {
      return;
    }
    ...
    unseatedPanel.innerHTML = `
      <h4>Unseated attending guests</h4>
      <p class="small">Drag a guest onto a table, or use the dropdown fallback.</p>
    `;
    ...
    unseated.querySelectorAll('.seat-row').forEach(row => {
      ...
      row.addEventListener('dragstart', event => {
        event.dataTransfer.setData('text/plain', row.dataset.guest);
      });
    });
    ...
    grid.querySelectorAll('.seat-card').forEach(card => {
      ...
      card.addEventListener('drop', event => {
        ...
        const select = unseated.querySelector(
          `.assign-select[data-guest="${cssEscape(guestId)}"]`
        );
        if (!select) {
          return;
        }
