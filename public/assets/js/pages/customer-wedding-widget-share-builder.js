(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .ww-seat-toolbox {
        ...
        border-radius: 18px;
        background: linear-gradient(135deg, rgba(255, 255, 255, 0.78), rgba(247, 253, 252, 0.68));
        box-shadow: 0 12px 28px rgba(36, 67, 111, 0.06);
      }
      .ww-advanced-panel__head {
        display: flex;
      ...
      .ww-advanced-grid select,
      .ww-advanced-grid textarea {
        width: 100%;
      ...
      .ww-qr-card canvas {
        ...
        border-radius: 16px;
        box-shadow: 0 12px 28px rgba(36, 67, 111, 0.08);
      }
      .ww-qr-fallback {
        display: grid;
        place-items: center;
  ...
  function enhanceUniqueLinkWording(root) {
    ...
    root.querySelector('#ww-copy-link')?.replaceChildren(
      document.createTextNode('Copy wedding website link')
    );
  }
  ...
  async function enhanceShare(root) {
    ...
    admin.innerHTML = `
      <div class="ww-advanced-panel__head">
        <div>
          <p class="ww-kicker">Share settings</p>
          <h4>Your unique wedding website link</h4>
          <p>Control the public link, privacy mode and guest sharing tools.</p>
        </div>
        <div class="ww-share-status-card"><span>Status</span><strong>${esc(site.status || 'draft')}</strong><small>${esc(buildVisibilityText(site))}</small></div>
      </div>
      <div class="ww-advanced-grid">
        <label>Custom website link<input id="ww-custom-slug" value="${esc(slug)}" placeholder="our-wedding"></label>
        <label>Privacy<select id="ww-visibility"><option value="private_link">Private link</option><option value="public">Public</option><option value="password">Password protected</option></select></label>
        <label>Website password<input id="ww-site-pass" type="password" placeholder="${site.passwordSet ? 'Leave blank to keep existing' : 'Set password'}"></label>
      </div>
      <span class="ww-link-preview">${esc(window.location.origin)}/wedding/<strong id="ww-slug-preview">${esc(slug)}</strong></span>
      <div class="ww-actions">
        <button type="button" class="cta secondary small" id="ww-save-share">Save share settings</button>
        <a class="cta secondary small" href="https://wa.me/?text=${encodeURIComponent(url)}" target="_blank" rel="noopener">Share to WhatsApp</a>
        <a class="cta secondary small" href="mailto:?subject=Wedding%20details&body=${encodeURIComponent(url)}">Share by email</a>
        <button type="button" class="cta secondary small" id="ww-download-qr">Download QR code</button>
      </div>
      <div class="ww-qr-card"><div id="ww-qr-target" class="ww-qr-fallback">QR loading…</div><p>Scannable QR code for your unique guest link.</p></div>
      <p class="small" id="ww-share-status" role="status" aria-live="polite"></p>`;
    actions.after(admin);
    ...
    slugInput.addEventListener('input', event => {
      const preview = normalizeSlug(event.target.value) || 'our-wedding';
      admin.querySelector('#ww-slug-preview').textContent = preview;
    });
  ...
  async function refreshSeatingTab(root) {
    const tab = root.closest('.ww-app-dialog')?.querySelector('.ww-app-tabs [data-tab="seating"]');
    tab?.click();
  }
  ...
  async function enhanceSeating(root) {
    ...
    toolbox.innerHTML = `
      <strong>Seating tools</strong>
      <div class="ww-actions">
        <button class="cta secondary small" id="ww-auto-seat" type="button">Auto-seat remaining</button>
        <button class="cta secondary small ww-btn-danger-soft" id="ww-clear-seats" type="button">Clear seating</button>
      </div>`;
    ...
    toolbox.querySelector('#ww-auto-seat')?.addEventListener('click', async event => {
      ...
      if (!guestIds.length) {
        toast('No unseated guests to auto-seat');
        return;
      }
      if (!slots.length) {
        toast('Add more table capacity first', 'warn');
        return;
      }
      button.disabled = true;
      try {
        const assignments = guestIds.slice(0, slots.length);
        for (let i = 0; i < assignments.length; i += 1) {
          await assignGuest(plan.id, slots[i], assignments[i]);
        }
  ...
  window.initWeddingWebsiteDashboard = async function initWeddingWebsiteDashboardWithAdvancedWeddingPolish(
    plans
  ) {
    cachedPlans = plans || [];
