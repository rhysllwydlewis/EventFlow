(function () {
  ...
  function overviewState(panel) {
    ...
    return {
      ...
      websiteStarted: !/not started/i.test(website),
      responses: rsvpParts[0] || 0,
      expected: rsvpParts[1] || guests || 0,
      guests,
      seated,
      url: panel.querySelector('.ww-share-mini span')?.textContent || '',
    };
  }
  ...
  function nextAction(state) {
    if (!state.websiteStarted) return { tab: 'workspace', title: 'Create your wedding website', body: 'Start the workspace so guests have one clear place for details and RSVPs.', button: 'Start setup' };
    if (!state.websitePublished) return { tab: 'workspace', title: 'Finish and publish your website', body: 'Complete the essentials, preview the guest page, then publish when ready.', button: 'Open workspace' };
    if (!state.guests) return { tab: 'guests', title: 'Add your first guests', body: 'Build your guest list so RSVPs and seating have something to work with.', button: 'Add guests' };
    if (state.expected && state.responses < state.expected) return { tab: 'guests', title: 'Track outstanding RSVPs', body: `${state.responses}/${state.expected} responses received. Keep an eye on guests still awaiting.`, button: 'Review RSVPs' };
    if (state.guests && state.seated < state.guests) return { tab: 'seating', title: 'Seat remaining guests', body: `${state.guests - state.seated} guest${state.guests - state.seated === 1 ? '' : 's'} still need a table.`, button: 'Open seating' };
    return { tab: 'share', title: 'Share your wedding website', body: 'Everything looks healthy. Copy the link or QR code and send it to guests.', button: 'Open share tools' };
  }
  ...
  function enhanceOverview(dialog) {
    ...
    if (!panel || panel.dataset.wwProductOverview === 'true') return;
  ...
  function enhanceWorkspace(dialog) {
    ...
    if (actions && !actions.querySelector('.ww-save-state')) {
      ...
      actions.querySelector('#ww-save')?.addEventListener('click', () => {
        ...
        window.setTimeout(() => { if (saveState.isConnected) saveState.textContent = '✓ Saved'; }, 900);
      });
    }
    pane.querySelectorAll('.rep-add').forEach(button => {
      if (!button.textContent.trim().startsWith('+')) button.textContent = `+ ${button.textContent.trim()}`;
    });
  }
  ...
  function enhanceGuests(dialog) {
    ...
    if (!table || pane.querySelector('.ww-guest-card-list')) return;
    ...
    cards.innerHTML = rows.length
      ? rows.map(row => {
          ...
          return `<article class="ww-guest-card"><div class="ww-guest-card__top"><div><strong>${esc(cells[0]?.textContent || 'Guest')}</strong><br><small>${esc(cells[6]?.textContent || 'manual')}</small></div>${cells[1]?.innerHTML || ''}</div><div class="ww-guest-card__meta"><span>Party ${esc(cells[2]?.textContent || '1')}</span><span>${esc(cells[5]?.textContent || 'No table')}</span>${cells[4]?.textContent ? `<span>${esc(cells[4].textContent)}</span>` : ''}</div><div class="ww-actions">${cells[8]?.innerHTML || ''}</div></article>`;
        }).join('')
  ...
  function enhanceSeating(dialog) {
    ...
    const panel = pane?.querySelector('.ww-app-panel');
    if (!panel || panel.querySelector('.ww-seating-command')) return;
  ...
  function installTabKeyboard(dialog) {
    ...
    if (!tabs.length || dialog.dataset.wwProductKeyboard === 'true') return;
