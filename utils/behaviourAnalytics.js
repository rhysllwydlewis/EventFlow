...
FUNNEL_STAGES.forEach((stage, index) => {
  stage.events.forEach(eventName => FUNNEL_STAGE_BY_EVENT.set(eventName, index));
});
...
function updateFunnelProgress(eventName, sessionKey, progressBySession, stageSessions) {
...
function buildSummary(rawEvents, days = 30, now = new Date()) {
  ...
  events.forEach(event => {
    ...
    updateFunnelProgress(event.event, sessionKey, funnelProgressBySession, funnelStageSessions);
