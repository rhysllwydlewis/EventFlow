...
function detectMimeFromBuffer(buffer) {
  ...
  // Plain text is intentionally permissive but excludes NUL/control-heavy data.
  ...
  if (nulCount === 0 && controlCount / sample.length < 0.02) return 'text/plain';
...
function validateMessengerAttachment(file) {
  ...
  if (!file.buffer || !Buffer.isBuffer(file.buffer)) {
    return { ok: false, reason: 'Attachment buffer missing' };
  }
  ...
  const isOfficeXml = ['application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'].includes(mimetype);
  ...
  const safeExtension = EXTENSION_BY_MIME_TYPE[mimetype] || originalExt || '.bin';
  return {
    ok: true,
    mimetype,
