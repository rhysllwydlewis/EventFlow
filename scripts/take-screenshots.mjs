import { chromium } from '/home/runner/work/EventFlow/EventFlow/node_modules/playwright-core/index.js';
import fs from 'fs';
...
const pages = [
  ...
  { slug: 'public--calendar',           path: '/public-calendar.html',            section: 'Public' },
  ...
  { slug: 'customer--dashboard-old',    path: '/dashboard.html',                  section: 'Customer' },
...
async function run() {
  ...
  for (const p of pages) {
    ...
    try {
      ...
      results.push({ ...p, file: p.slug + '.png', ok: true, bytes: size });
      console.log('✓', p.slug, `(${Math.round(size/1024)}KB)`);
    } catch (e) {
      results.push({ ...p, ok: false, error: e.message.slice(0, 120) });
  ...
  console.log(`\nDone: ${ok}/${results.length} screenshots saved to ${OUT}`);
}
