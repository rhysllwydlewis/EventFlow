...
test.describe("Current account-first supplier onboarding @backend", () => {
  ...
  test("reveals supplier account essentials without recreating the retired full-profile form", async ({
    ...
    await expect(
      page.locator("#sup-category, #sup-long, .gallery-upload"),
    ).toHaveCount(0);
  });

  test("keeps supplier-specific fields out of the customer account path", async ({
    page,
  }) => {
    await page.goto("/auth");
    await page.locator("#tab-create").click();
  ...
  test("blocks login until account email verification is complete", async ({
    request,
  }) => {
