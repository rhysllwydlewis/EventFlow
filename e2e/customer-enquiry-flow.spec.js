...
test.describe("Customer enquiry journey against the real backend @backend", () => {
  ...
  test("serves the approved supplier on its clean public URL", async ({
    page,
  }) => {
  ...
  test("gates a logged-out enquiry with a login-aware message surface", async ({
    ...
    await expect
      .poll(async () => {
        ...
        return (
          state.authRedirect || state.messengerRedirect || state.dialogVisible
        );
      })
      .toBe(true);
    ...
    if (new URL(page.url()).pathname === "/auth") {
      ...
      await expect(page.locator('[role="dialog"]:visible')).toContainText(
        /log in|message|enquir/i,
      );
    }
  });

  test("opens the real compose journey for an authenticated customer", async ({
    page,
  }) => {
    ...
    if (new URL(page.url()).pathname.startsWith("/messenger")) {
      const url = new URL(page.url());
      expect(url.searchParams.get("recipientId")).toBe(
        fixtures.users.supplier.id,
      );
