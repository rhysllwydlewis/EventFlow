...
async function enquirySurface(page) {
  const dialog = page.locator('[role="dialog"]:visible').filter({ hasText: /log in|message|enquir/i });
  return {
...
test.describe('Customer enquiry journey against the real backend @backend', () => {
  ...
  test('gates a logged-out enquiry with a login-aware message surface', async ({ page, context }) => {
