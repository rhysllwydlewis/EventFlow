...
test.describe('Admin feature flags against the real backend @backend', () => {
  ...
  test('persists critical toggles, enforces them, and restores the original state', async ({ page }) => {
    ...
    try {
      ...
      await expect(customerAttempt.json()).resolves.toMatchObject({ feature: 'registration' });
      ...
      await expect(supplierAttempt.json()).resolves.toMatchObject({ feature: 'supplierApplications' });
  ...
  test('renders the current server state in the admin settings UI', async ({ page }) => {
    ...
    expect(await page.locator('#featureRegistration').isChecked()).toBe(flags.registration !== false);
