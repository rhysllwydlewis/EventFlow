...
(() => {
  ...
  function buildAdminTransitionGuard() {
    ...
    guardRouter.patch('/:id', async (req, res, next) => {
      ...
      if (!['approved', 'processing', 'delivered', 'rejected'].includes(targetStatus))
        return next();
      try {
        const request = await strictStore.findOne('partner_cashout_requests', {
          id: req.params.id,
        });
          ...
          if (!decision.allowed) {
            if (
              ...
              notifyAdmins({
                ...
                logger.warn(
                  '[PARTNER-CASHOUT-OPS] Non-blocking administrator notification failed',
                  {
                    error: error.message,
                  }
                );
