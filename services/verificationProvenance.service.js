...
function inferVerifiedBy(user, method) {
  ...
  if (method === 'admin_created') {
    return {
      type: 'admin',
      userId: user && user.createdBy ? user.createdBy : null,
      reason: 'Account was created by an admin',
    };
  }
...
function findVerificationLogsForUser(user, logs) {
  ...
  return (logs || [])
    ...
    .sort((a, b) =>
      String(b.createdAt || b.attemptedAt || '').localeCompare(
        String(a.createdAt || a.attemptedAt || '')
      )
    );
}

function inferEmailDeliveryStatus(user, logs) {
  if (user && user.emailDeliveryStatus) {
  ...
  if (matching.some(log => log.status === 'bounced')) {
    return 'bounced';
  }
  if (matching.some(log => log.status === 'failed')) {
    return 'failed';
  }
  if (matching.some(log => log.provider === 'outbox')) {
    return 'outbox';
  }
  if (matching.some(log => ['sent', 'delivered', 'opened', 'clicked'].includes(log.status))) {
    return 'sent';
  }
...
function summariseUser(user, logs) {
  ...
  return {
    ...
    verificationEmailSentAt:
      iso(user && user.verificationEmailSentAt) || iso(lastLog && lastLog.sentAt),
    lastVerificationEmailLogId:
      (user && user.lastVerificationEmailLogId) || (lastLog && lastLog.id) || null,
    lastVerificationEmailPostmarkMessageId:
      (user && user.lastVerificationEmailPostmarkMessageId) ||
      (lastLog && lastLog.postmarkMessageId) ||
      null,
...
async function getVerificationIntegrity(options) {
  ...
  const logsInLookback = logs.filter(log => {
    const created = Date.parse(log.createdAt || log.attemptedAt || log.sentAt || 0);
  ...
  if (production && !postmarkConfigured) {
    addIssue(
      ...
      'critical',
      null,
      'postmark_not_configured_in_production',
      'Postmark is not configured in production.'
    );
  }
  if (production && !emailEnabled) {
    addIssue(
      ...
      'critical',
      null,
      'email_disabled_in_production',
      'Email sending is disabled in production.'
    );
  }
  ...
  for (const user of checkedUsers) {
    ...
    const userLogs = findVerificationLogsForUser(user, logs);
    if (
      summary.signupMethod === 'email_password' &&
      summary.verificationMethod !== 'not_required' &&
      userLogs.length === 0
    ) {
      addIssue(
        issues,
        ...
        'Email/password user has no verification email log.'
      );
    }
    if (user.verified === true && !summary.verifiedAt) {
      addIssue(
        ...
        'warning',
        user,
        'verified_user_missing_verified_at',
        'Verified user has no verifiedAt timestamp.',
        { verificationMethod: summary.verificationMethod }
      );
    }
    if (
      user.verified === true &&
      (!summary.verificationMethod || summary.verificationMethod === 'unknown')
    ) {
      addIssue(
        issues,
        'warning',
        user,
        'verified_user_unknown_verification_method',
        'Verified user has unknown verification method.'
      );
    }
    if (summary.verificationMethod === 'eventflow_email' && !summary.verifiedAt) {
      addIssue(
        issues,
        'warning',
        user,
        'eventflow_email_verification_missing_verified_at',
        'EventFlow-email verified user has no verifiedAt timestamp.'
      );
    }
    if (user.verified === true && summary.signupMethod === 'unknown') {
      addIssue(
        ...
        'warning',
        user,
        'verified_user_unknown_signup_method',
        'Verified user has unknown signup method.'
      );
    }
    if (user.authProvider === 'google' && !hasGoogleLink(user)) {
      addIssue(
        issues,
        'critical',
        user,
        'google_provider_missing_google_link',
        'Google-authenticated user has no Google link field.'
      );
    }
    if (hasGoogleLink(user) && !['google', 'mixed'].includes(user.authProvider)) {
      addIssue(
        issues,
        'warning',
        user,
        'google_link_with_non_google_provider',
        'User has Google linkage but stored authProvider is not google or mixed.',
    ...
    for (const log of userLogs) {
      if (production && log.provider === 'outbox') {
        addIssue(
          ...
          'critical',
          user,
          'verification_email_logged_as_outbox_in_production',
          'Verification email used outbox in production.',
          { emailLogId: log.id }
        );
      }
      if (log.status === 'failed') {
        addIssue(
          ...
          { emailLogId: log.id, errorMessage: log.errorMessage || null }
        );
      }
      if (log.status === 'bounced') {
        addIssue(
          ...
          'critical',
          user,
          'verification_email_bounced',
          'Verification email bounced.',
          { emailLogId: log.id }
        );
      }
    }
  ...
  if (checkedUsers.length > 0 && logsInLookback.length === 0) {
    addIssue(
      issues,
      'warning',
      null,
      'recent_users_but_no_recent_verification_logs',
      'Recent users exist but no verification email logs were found.',
      { checkedUsers: checkedUsers.length }
    );
  }

  const sortedLogs = logsInLookback
    .slice()
    .sort(
      (a, b) =>
  ...
  const successfulLogs = sortedLogs.filter(log =>
    ['sent', 'delivered', 'opened', 'clicked'].includes(log.status)
  );
  const failedLogs = sortedLogs.filter(log => ['failed', 'bounced'].includes(log.status));
  ...
  const summary = {
    ...
    emailPasswordUsersMissingVerificationEmailLogs: issues.filter(
      issue => issue.issue === 'email_password_user_missing_verification_email_log'
    ).length,
    ...
    bouncedVerificationEmailLogs: logsInLookback.filter(log => log.status === 'bounced').length,
    lastVerificationEmailAttemptAt: iso(
      sortedLogs[0] &&
        (sortedLogs[0].sentAt || sortedLogs[0].createdAt || sortedLogs[0].attemptedAt)
    ),
    lastSuccessfulVerificationEmailAt: iso(
      successfulLogs[0] &&
        (successfulLogs[0].sentAt || successfulLogs[0].createdAt || successfulLogs[0].attemptedAt)
    ),
    lastFailedVerificationEmailAt: iso(
      failedLogs[0] &&
        (failedLogs[0].sentAt || failedLogs[0].createdAt || failedLogs[0].attemptedAt)
    ),
  };
