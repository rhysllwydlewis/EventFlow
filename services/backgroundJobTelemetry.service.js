...
const JOB_KEYS = Object.freeze({
  ...
  BROWSE_NUDGE: 'browse-nudge',
  NEWSLETTER_CADENCE: 'newsletter-cadence',
});
...
function getDefinitions(now = new Date()) {
  ...
  return [
    ...
    {
      key: JOB_KEYS.VERIFICATION_REMINDER,
      name: 'Email verification reminders',
      description: 'Nudges users who signed up but never verified their email (max 2 reminders).',
      source: 'services/emailVerificationReminderScheduler.js',
      schedule: process.env.EMAIL_VERIFICATION_REMINDER_CRON || '40 9 * * *',
      scheduleSource: process.env.EMAIL_VERIFICATION_REMINDER_CRON ? 'environment' : 'default',
      enabled: envEnabled('EMAIL_VERIFICATION_REMINDER_ENABLED', production),
      expectedIntervalMs: 24 * 60 * 60 * 1000,
      staleAfterMs: 36 * 60 * 60 * 1000,
      legacyTelemetry: null,
      legacyFreshnessReliable: false,
    ...
    {
      key: JOB_KEYS.BROWSE_NUDGE,
      name: 'Browse-supplier nudges',
      description:
        'Emails customers who saved suppliers but never made an enquiry, pointing them back to what they saved.',
      source: 'services/customerBrowseNudgeScheduler.js',
      schedule: process.env.BROWSE_NUDGE_CRON || '15 10 * * *',
      scheduleSource: process.env.BROWSE_NUDGE_CRON ? 'environment' : 'default',
      enabled: envEnabled('BROWSE_NUDGE_ENABLED', production),
      expectedIntervalMs: 24 * 60 * 60 * 1000,
      staleAfterMs: 36 * 60 * 60 * 1000,
      legacyTelemetry: null,
      legacyFreshnessReliable: false,
    ...
    {
      key: JOB_KEYS.NEWSLETTER_CADENCE,
      name: 'Automated newsletter cadence',
      description:
        'Sends the admin-configured recurring newsletter (weekly/monthly) when it falls due.',
      source: 'services/newsletterCadenceScheduler.js',
      schedule: process.env.NEWSLETTER_CADENCE_CRON || '30 8 * * *',
      scheduleSource: process.env.NEWSLETTER_CADENCE_CRON ? 'environment' : 'default',
      enabled: envEnabled('NEWSLETTER_CADENCE_ENABLED', production),
      expectedIntervalMs: 24 * 60 * 60 * 1000,
      staleAfterMs: 36 * 60 * 60 * 1000,
      legacyTelemetry: null,
      legacyFreshnessReliable: false,
