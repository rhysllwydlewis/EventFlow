...
router.get('/public/wedding-websites/:slug', async (req, res) => {
  ...
  if (site.visibility === 'password' && !hasWeddingPasswordAccess(req, site)) {
    return res.status(401).json({ error: 'Password required', passwordRequired: true, noindex: true });
  }
...
router.post('/public/wedding-websites/:slug/access', passwordAccessLimiter, async (req, res) => {
  ...
  if (!password || !site.passwordHash || !verifyWeddingPassword(password, site.passwordHash)) {
    return res.status(403).json({ error: 'That password was not recognised.', passwordRequired: true });
  }
...
router.post('/public/wedding-websites/:slug/rsvp', writeLimiter, async (req, res) => {
  ...
  if (!(attending === true || attending === false || attending === 'true' || attending === 'false')) {
  ...
  if (guest) {
    ...
    list.push({ id: uid('guest'), planId: plan.id, userId: plan.userId, ...patch, rsvpSubmittedAt: now, createdAt: now });
  }
  await dbUnified.updateOne('plans', { id: plan.id }, { $set: { ...setGuestList(plan, list), updatedAt: now } });
...
router.get('/:planId/tables', authRequired, getOwnedPlan, async (req, res) => res.json({ success: true, tables: getTables(req.plan) }));
router.post('/:planId/tables', authRequired, requireVerifiedUser, csrfProtection, writeLimiter, getOwnedPlan, async (req, res) => {
  const now = new Date().toISOString();
  const t = { id: uid('table'), planId: req.plan.id, name: sanitize(req.body.name || 'Table', 80), type: req.body.type || 'round', capacity: Math.max(1, Math.min(30, Number(req.body.capacity) || 10)), guestIds: [], notes: sanitize(req.body.notes, 500), createdAt: now, updatedAt: now };
...
router.delete('/:planId/tables/:tableId', authRequired, requireVerifiedUser, csrfProtection, writeLimiter, getOwnedPlan, async (req, res) => {
  ...
  const now = new Date().toISOString();
  await dbUnified.updateOne('plans', { id: req.plan.id }, { $set: { tables, ...setGuestList(req.plan, guests), updatedAt: now } });
  res.json({ success: true });
});
router.post('/:planId/tables/:tableId/assign-guest', authRequired, requireVerifiedUser, csrfProtection, writeLimiter, getOwnedPlan, async (req, res) => {
  const tableId = req.params.tableId;
  ...
  await dbUnified.updateOne('plans', { id: req.plan.id }, { $set: { tables, [Array.isArray(req.plan.guests) ? 'guests' : 'guestList']: guests, updatedAt: new Date().toISOString() } });
  res.json({ success: true });
});
router.post('/:planId/tables/unassign-guest', authRequired, requireVerifiedUser, csrfProtection, writeLimiter, getOwnedPlan, async (req, res) => {
  const guestId = req.body.guestId;
  const tables = getTables(req.plan);
  tables.forEach(tb => (tb.guestIds = (tb.guestIds || []).filter(id => id !== guestId)));
  const guests = guestListForPlan(req.plan).map(g => g.id === guestId ? { ...g, tableId: null, tableName: null, updatedAt: new Date().toISOString() } : g);
  await dbUnified.updateOne('plans', { id: req.plan.id }, { $set: { tables, [Array.isArray(req.plan.guests) ? 'guests' : 'guestList']: guests, updatedAt: new Date().toISOString() } });
  res.json({ success: true });
});
router.get('/:planId/seating-summary', authRequired, getOwnedPlan, async (req, res) => {
  ...
  const tables = getTables(req.plan);
  res.json({ success: true, summary: { tables: tables.length, seated: attending.filter(g => g.tableId || g.tableName || g.table).length, unseated: attending.filter(g => !(g.tableId || g.tableName || g.table)).length, overCapacity: tables.filter(t => (t.guestIds || []).length > Number(t.capacity || 0)).length } });
});
