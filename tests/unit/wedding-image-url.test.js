...
describe('wedding image URL sanitiser', () => {
  test('allows http, https and valid base64 data image URLs', () => {
    expect(sanitizeWeddingImageUrl('https://example.com/photo.jpg')).toBe(
      'https://example.com/photo.jpg'
    );
    expect(sanitizeWeddingImageUrl('http://example.com/photo.jpg')).toBe(
      'http://example.com/photo.jpg'
    );
    expect(sanitizeWeddingImageUrl('data:image/png;base64,aaaa')).toBe(
  ...
  test('blocks unsafe, malformed and unsupported image URLs', () => {
    ...
    expect(
      sanitizeWeddingImageUrl("data:image/png;base64,abc');background:url(javascript:alert(1))")
    ).toBe('');
